...
{
	if(bufferSizeBytes < queryTimeMessageUdpSize(message))
	{
		return JAUS_FALSE; // improper size
	}
	else
	{
		...
		return queryTimeMessageToBuffer(message, buffer+JAUS_UDP_HEADER_SIZE_BYTES, bufferSizeBytes - JAUS_UDP_HEADER_SIZE_BYTES);
	}
}
