...
{
	if(bufferSizeBytes < queryComponentStatusMessageUdpSize(message))
	{
		return JAUS_FALSE; // improper size
	}
	else
	{
		...
		return queryComponentStatusMessageToBuffer(message, buffer+JAUS_UDP_HEADER_SIZE_BYTES, bufferSizeBytes - JAUS_UDP_HEADER_SIZE_BYTES);
	}
}
