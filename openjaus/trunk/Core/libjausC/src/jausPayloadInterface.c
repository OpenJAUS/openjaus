...
JausPayloadInterface jausPayloadInterfaceCreate(void)
{
	...
	if(payloadInterface)
	{
		...
		payloadInterface->jausCommandInterfaces = vectorCreate();		// Dynamic Array of Input Commands
		payloadInterface->jausInformationInterfaces = vectorCreate();	// Dynamic Array of Output Commands
...
void jausPayloadInterfaceDestroy(JausPayloadInterface payloadInterface)
{
	if(payloadInterface)
	{
		vectorDestroy(payloadInterface->jausCommandInterfaces, (void *)jausCommandInterfaceDestroy);
		vectorDestroy(payloadInterface->jausInformationInterfaces, (void *)jausInformationInterfaceDestroy);
...
JausCommandInterface jausAddNewCommandInterface(JausPayloadInterface payloadInterface, char* identifier, JausByte typeCode)
{
	...
	if( (commandInterface = jausCommandInterfaceCreate() ) )
	{
		...
		vectorAdd(payloadInterface->jausCommandInterfaces, commandInterface);
		return commandInterface;
	}
	else
	{
...
JausInformationInterface jausAddNewInformationInterface(JausPayloadInterface payloadInterface, char* identifier, JausByte typeCode)
{
	...
	if( (informationInterface = jausInformationInterfaceCreate() ) )
	{
		...
		vectorAdd(payloadInterface->jausInformationInterfaces, informationInterface);	
		return informationInterface;
	}
	else
	{
...
JausBoolean jausSetCommandInterfaceEnumeration(JausPayloadInterface payloadInterface, char* identifier, char* enumeration)
{
	...
	if( (commandInterface = jausCommandInterfaceRetrieve(payloadInterface, identifier) ) )
	{
		...
		commandInterface->enumerationLength = strlen(enumeration) + 1; // include the NULL character
		return JAUS_TRUE;
	}
	else
	{
...
JausBoolean jausSetInformationInterfaceEnumeration(JausPayloadInterface payloadInterface, char* identifier, char* enumeration)
{
	...
	if( (informationInterface = jausInformationInterfaceRetrieve(payloadInterface, identifier) ) )
	{
		...
		informationInterface->enumerationLength = strlen(enumeration) + 1; // include the NULL character
		return JAUS_TRUE;
	}
	else
	{
...
int jausInformationValueToBuffer(JausInformationInterface informationInterface, unsigned char* buffer, unsigned int bufferSizeBytes)
{
	...
	switch((int) informationInterface->typeCode)
	{
		...
		case TYPE_CODE_SCALED_U_SHORT:
			...
			if(jausFloatToBuffer(tempUShort, buffer, bufferSizeBytes) ) return JAUS_UNSIGNED_SHORT_SIZE_BYTES;
			else return 0;
			break;
		...
		case TYPE_CODE_SCALED_INTEGER:
			...
			if(jausFloatToBuffer(tempInt, buffer, bufferSizeBytes) ) return JAUS_INTEGER_SIZE_BYTES;
			else return 0;
			break;
		...
		case TYPE_CODE_SCALED_U_INTEGER:
			...
			if(jausFloatToBuffer(tempUInt, buffer, bufferSizeBytes) ) return JAUS_UNSIGNED_INTEGER_SIZE_BYTES;
			else return 0;
			break;
		...
		case TYPE_CODE_SCALED_LONG:
			...
			if(jausFloatToBuffer(tempLong, buffer, bufferSizeBytes) ) return JAUS_LONG_SIZE_BYTES;
			else return 0;
			break;
		...
		case TYPE_CODE_SCALED_U_LONG:
			...
			if(jausFloatToBuffer(tempULong, buffer, bufferSizeBytes) ) return JAUS_UNSIGNED_LONG_SIZE_BYTES;
			else return 0;
			break;
